    ...
    <>
        ...
        <div className="flex flex-wrap justify-center font-bold text-lg sm:text-xl lg:text-2xl gap-3 sm:gap-4 pt-12">
          {[
            ...
            <Link
              href={`#${day}`}
              key={day}
              className="transition duration-300 text-white hover:text-green-500"
            >
              {day}
            </Link>
          ))}
      ...
      <main className="container px-4 sm:px-6 lg:px-8 mx-auto py-8">
        {data.map(({ day, data: seriesData }) => (
          <div key={day} id={day} className="target:bg-green-400 w-full py-2">
            <h2 className="text-xl sm:text-2xl mt-3 font-semibold mb-4">
              # {day}
            </h2>
